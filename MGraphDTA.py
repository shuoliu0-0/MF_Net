...
def get_batch(ligand_batch):
    batch = []
    num_graph = len(ligand_batch)
    for i in range(num_graph):
        batch += [i ] *int(ligand_batch[i])
    batch = torch.tensor(batch).to(ligand_batch.device)
    return batch
...
class TargetRepresentation(nn.Module):
    def __init__(self, block_num, embedding_num, out_dim):
        super().__init__()
        ...
        self.linear = nn.Linear(block_num * 96, out_dim)
...
class GraphConvBn(nn.Module):
    ...
    def forward(self, data):
        ...
        data['x'] = F.relu(self.norm(self.conv(x.float(), edge_index)))
...
class GraphDenseNet(nn.Module):
    ...
    def forward(self, data0):
        batch = get_batch(data0.batch_num_nodes())
        edge_index = torch.stack(data0.edges(), dim=0)
        data = {'batch' :batch, 'x' :data0.ndata['h'], 'edge_attr' :data0.edata['e'] ,'edge_index' :edge_index}
