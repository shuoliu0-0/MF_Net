...
parser.add_argument('--log_url', default='/home/cuda/nfs3/lius/code/MF_Net/output/train.txt', help='Location of logs.')
# parser.add_argument('--log_url', default='/home/cuda/nfs3/lius/code/dock2-edis/output/1train40.log', help='Location of logs.')
parser.add_argument('--echart_ori_url', default='/home/cuda/nfs3/lius/code/dock2/echarts',
                    help='Location of echarts_loss_logs.')
parser.add_argument('--out_url', default='/home/cuda/nfs3/lius/code/dock2-edis/out_loss', help='output result.')
...
while True:
    ...
    for h, x in enumerate(log_data):
        ...
        # if 'train: ' in x:
        # if 'test: ' in x:
        # if 'test ensemble: ' in x:
        # if 'v2013test ensemble: ' in x:
        # if 'v2019test ensemble: ' in x:
        if '0test ensemble: ' in x:
            # x = x.split('epoch')[1]
            # step = int(x.split('= ')[0].strip('\n'))
            # loss = float(x.split()[4].strip('\n'))
            # loss = float(x.split()[3].strip('\n')) #MAE
            loss = float(x.split()[5].strip('\n')) #RMSE
            # loss = float(x.split()[7].strip('\n')) #pr
            # loss = float(x.split()[9].strip('\n')) #SD
            # loss = float(x.split()[11].strip('\n')) #R2
    ...
    print("finished process log file: ", str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
